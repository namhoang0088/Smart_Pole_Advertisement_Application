...
// Dữ liệu giả lập
const humidityData = [45, 55, 62, 48, 50, 75, 85, 90, 67, 72, 49, 50, 65, 88, 40, 30, 55, 70, 82, 45, 55, 60, 72, 68];
const temperatureData = [15, 18, 20, 22, 25, 28, 30, 35, 32, 29, 26, 22, 19, 15, 10, 8, 12, 18, 25, 30, 35, 32, 27, 20];
const PM25Data = [10, 20, 35, 40, 60, 75, 80, 50, 25, 30, 45, 55, 65, 70, 40, 35, 50, 60, 75, 85, 90, 95, 100, 110];
const PM10Data = [15, 30, 45, 50, 65, 80, 85, 55, 30, 35, 50, 60, 70, 75, 45, 40, 55, 65, 80, 90, 95, 100, 105, 115];
const noiseData = [20, 35, 45, 55, 70, 80, 85, 50, 40, 30, 60, 75, 80, 65, 55, 50, 65, 75, 85, 90, 95, 100, 105, 110];
const luxData = [50, 100, 200, 300, 400, 500, 600, 300, 200, 100, 250, 350, 450, 550, 300, 200, 350, 450, 550, 600, 650, 700, 750, 800];
...
export default function AirQualityHistory({ open, handleClose }) {
  ...
  const maxHumidity = Math.max(...humidityData) + Math.max(...humidityData)*0.1;
  const maxTemperature = Math.max(...temperatureData) + Math.max(...temperatureData)*0.1;
      ...
      tooltip: {
        callbacks: {
          label: function (context) {
            ...
            label += context.raw + (context.dataset.label.includes('dB') ? ' dB' : context.dataset.label.includes('lux') ? ' lux' : ' µg/m³');
            return label;
          }
        }
      }
